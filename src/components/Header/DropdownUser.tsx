...
import { logLogout, setUserActivities } from '../../state/Slices/ActivitySlice';
...
const DropdownUser = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        setJobTitle(userResponse.user.role);
      } catch (error) {
  ...
  const logout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestBody = JSON.stringify({ userActivities: updatedUserActivities });
      const response = await fetch(`${baseUrl.baseUrl}/userActivity/addAllActivity/${username}`, {
        method: 'POST',
        headers: {
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{jobTitle}</span>
        </span>
